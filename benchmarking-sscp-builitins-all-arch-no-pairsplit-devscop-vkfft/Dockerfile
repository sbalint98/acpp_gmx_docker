...
ARG ACPP_VERSION=github.com:sbalint98/AdaptiveCpp.git#bsoproni-all-gmx-builtins-barrier-wgscope

ADD git@$ACPP_VERSION ./AdaptiveCpp

# GMX variants
ADD git@gitlab.com:sbalint98/gromacs.git#sb-rel25-sscp-enable-upstream ./gromacs-smcp
ADD git@gitlab.com:sbalint98/gromacs.git#enable-sscp-builtins-all-arch-fft ./gromacs-sscp-builtins
...
ARG ACPP_CMAKE_OPTIONS=" $GENERAL_CMAKE_OPTIONS \ 
                         -D GMX_GPU_NB_DISABLE_CLUSTER_PAIR_SPLIT=ON \
...
# Setting up Profile environment
RUN apt update
RUN apt -y install rocprofiler-compute python3.12-venv locales
RUN pip install --ignore-installed --break-system-packages  -r /opt/rocm-6.4.1/libexec/rocprofiler-compute/requirements.txt
